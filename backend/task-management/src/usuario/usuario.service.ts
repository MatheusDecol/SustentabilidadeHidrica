...
export class UsuarioService {
  ...
  async cadastrar(novoUsuario: Usuario): Promise<Usuario> {
    ...
    const resultado: UsuarioEntity | undefined =
    ...
    console.log('DTO recebido:', novoUsuario);
    console.log('Email extraído:', novoUsuario.getEmail);
    console.log('Senha extraída:', novoUsuario.getSenha);

    return UsuarioMapper.entityToDomain(
      await this.usuarioRepository.save(
        UsuarioMapper.domainToEntity(novoUsuario),
      ),
    );
  }
  ...
  async confirmarCadastro(token: string) {
    try {
      ...
      const usuarioAtualizado: Usuario = await this.buscarPorEmail(
        payload.email,
      );
